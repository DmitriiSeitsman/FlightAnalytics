...
import { DeviationsView } from "./deviations-view";

type View = "aircraftType" | "departure" | "arrival" | "flights" | "pilots" | "statistics" | "events" | "deviations";
  ...
  return <main>
    ...
    <section className="workspace">
      ...
      {!result ? (!loading ? <div className="feature-grid">
        ...
        <div className="result-strip"><div><span>Исходных строк</span><strong>{result.sourceRows.toLocaleString("ru-RU")}</strong></div><div><span>Уникальных рейсов</span><strong>{result.flights.length.toLocaleString("ru-RU")}</strong></div><div><span>Устранено дублей</span><strong>{result.duplicatesRemoved.toLocaleString("ru-RU")}</strong></div><p>{result.conflictingDuplicateGroups > 0 ? `В ${result.conflictingDuplicateGroups} группах дублей расхождения усреднены.` : "Расхождений внутри дублей не найдено."}{result.routeConflictGroups > 0 && ` В ${result.routeConflictGroups} группах дублей отличался аэропорт или тип ВС — выбран наиболее частый вариант.`}</p></div>
        ...
        <nav className="tabs" aria-label="Разрез аналитики">{([["aircraftType", "Типы ВС"], ["departure", "Аэродромы взлёта"], ["arrival", "Аэродромы посадки"], ["flights", "Рейсы"], ["pilots", "Пилоты"], ["statistics", "Статистика"], ...(result?.events && result.events.length > 0 ? [["events", "События"] as [View, string], ["deviations", "Отклонения"] as [View, string]] : [])] as Array<[View, string]>).map(([key, label]) => <button type="button" className={view === key ? "active" : ""} key={key} onClick={() => setView(key)}>{label}</button>)}</nav>
        {view !== "statistics" && view !== "flights" && view !== "deviations" && <p className="sort-help">Отметьте галочками нужные столбцы. Цифры показывают порядок сортировки; стрелка меняет направление.<span className="mobile-table-hint">↔ Проведите по таблице влево, чтобы увидеть остальные столбцы.</span></p>}
        {view === "statistics" ? <StatisticsView flights={flights} sourceFile={fileName} aircraftFilter={aircraftType} airportFilter={airport} positions={positions} /> : view === "deviations" ? <DeviationsView flights={flights} positions={positions} onSelectPilot={openPilotCard} /> : view === "events" ? <EventsAnalytics flights={flights} events={result?.events || []} aircraftFilter={aircraftType} airportFilter={airport} /> : view === "flights" ? <FlightsView flights={flights} positions={positions} onSelectPilot={openPilotCard} /> : view === "pilots" ? <>
